...
-- Not for the plans, for the one test below that needs a worker to actually
-- start.  A server configured with max_parallel_workers = 0 plans the Gather
-- and launches nobody, and ExecGather then runs the subplan in the leader
-- whatever parallel_leader_participation says - which turns that test into a
-- row count.  This is a USERSET GUC, so installcheck against such a server is
-- covered; max_worker_processes is not, being PGC_POSTMASTER, and a server
-- with no worker slots left will still produce a spurious difference there.
...
--
-- ... and it comes back when the extension is created again, without
-- reconnecting.  The pg_proc invalidation drops the negative answer just as it
-- dropped the positive one, and this is the order a deployment actually takes:
-- the library is preloaded, the sessions are already open, and somebody runs
-- CREATE EXTENSION afterwards.  Were the failed lookup remembered, every
-- existing connection would stay inert with nothing in the log to say so.
--
CREATE EXTENSION enforce_workers;
...
--
-- The replacement is looked up in the schema the extension was created in,
-- not through search_path.  That is not fastidiousness: the statements this
-- rewrites belong to an application that sets its own search_path and has
-- never heard of this module.
--
DROP EXTENSION enforce_workers;
CREATE SCHEMA sg_ns;
CREATE EXTENSION enforce_workers SCHEMA sg_ns;

SET search_path = pg_catalog;

EXPLAIN (COSTS OFF)
INSERT INTO pg_temp.sg_tmp_dst (a) SELECT a FROM public.sg_src;

RESET search_path;

-- Moving the extension afterwards does not strand the cached OID either.
ALTER EXTENSION enforce_workers SET SCHEMA public;
